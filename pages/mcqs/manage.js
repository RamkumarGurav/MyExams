...
const ManageMcqs = ({ initialData }) => {
  ...
  const { data, isLoading, isError,error,refetch } = useQuery(["manage-mcqs",pageIndex], ()=>fetcher(pageIndex),{
...
const {
  ...
  deleteFetcher,
  {
    onSuccess: (data) => {
      setBtnLoading(false);
      refetch();
      toast.success("Successfully Deleted the MCQ");
    },
    onError: (err) => {
      setBtnLoading(false);
...
//--------------------------------------------------------
  return (
    <Layout home gray>
      <Head>
        <title>Manage MCQS | MyExams.com</title>
      </Head>
      <h1 className="text-center mx-auto text-xl  cg-safron text-white  px-6 rounded-lg py-2 my-4 w-[200px] ">
        Manage MCQS
      </h1>
      {/* <h1 className="mx-auto w-[50%] text-gray-900 font-semibold text-center pt-4 text-2xl  border-b-2 border-gray-200">
        ALL MCQS
      </h1> */}
      <div className="flex flex-col sm:flex-row items-center justify-around gap-4 px-10">
        <div className="">
          <select
            name="subject"
            value={subject}
      ...
      return (<Question data={...question} qno={i+((pageIndex-1)*resultsPerPage)} key={i}  handleDeleteMCQ={handleDeleteMCQ}  btnLoading={btnLoading}/>)
      }))}
...
function Question({data,qno,handleDeleteMCQ,btnLoading}){
 ...
 const {questionK,question,answersK,answers,answerIndex,_id}=data
  ...
  return (
    <div className="q_block container px-2 py-4 bg-white border-2 my-2">
    <div className="flex">
   ...
   answers.map((a,i)=>(
    ...
    <div className="showAnswer flex justify-between items-center  w-ful bg-white  ">
      ...
      <button
            className="bg-[red]  flex justify-center items-center hover:bg-[#ff0000d3] text-white uppercase  text-sm py-2 px-2 sm:px-4 rounded-full focus:outline-none focus:shadow-outline my-4"
          onClick={()=>handleDeleteMCQ(_id)}  >
  Delete {qId === _id && btnLoading ? <ButtonLoader /> : null}

          </button>

           <Link href={`/mcqs/update/${_id}`}
            className="bg-[#22ce22]  flex justify-center items-center hover:bg-[#22ce22d3] text-white uppercase  text-sm py-2 px-2 sm:px-4 rounded-full focus:outline-none focus:shadow-outline my-4 "
           >Edit</Link>
      </div>
...
export async function getServerSideProps() {
  const res = await fetch(
    `${process.env.NEXT_PUBLIC_SERVER_URL}/questions?&page=1&limit=20`
  );
  const data = await res.json();
